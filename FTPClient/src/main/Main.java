...
public class Main {
	...
	private static final String USAGE = "사용법 : java -jar FTPClient.jar <ftp or sftp> <server> [port]";
	private static String server = null;
	private static int port = 21;
	private static String ftpType = null;
	
	public static void main(String args[]) {
		
		boolean result = argsGetParams(args);
		if(!result)
			return;
		
		FTPClientCommon ftp = FTPClientCommonFactory.getFtpClientCommon(ftpType, server, port);
		if(ftp == null){
			System.out.println("입력 파라미터를 확인해주세요.");
			return;
		}
		ftp.connect();
	...
	/* 서버 ip와 포트 입력 처리*/
	private static boolean argsGetParams(String[] args){
